...
/**
 * @brief the degree of btree	b树的度为M
 * key per node: [M-1, 2M-1]	有M-1到2M-1个关键字
 * child per node: [M, 2M]		有M到2M个子结点指针
 */
#define M 2
...
/**
 * @brief allocate a new btree node 申请一个新的b树结点空间
 * default: is_leaf == true
 *
 * @return pointer of new node 返回新结点的指针
 */
btree_node *btree_node_new();


/**
 * @brief create a btree root 创建b树的根结点
 *
 * @return pointer of btree root 返回根结点的指针
 */
